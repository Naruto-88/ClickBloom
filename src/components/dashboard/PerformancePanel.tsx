...
function draw(ctx: CanvasRenderingContext2D, data: Point[], active: Active, hoverIndex: number | null){
  ...
  function lineSmooth(values:number[], color:string, opts?:{fill?:boolean, dashed?:boolean}){
    ...
    if(max===min){
      ...
      // markers
      for(let i=0;i<values.length;i++){ const x=xs(i); ctx.fillStyle=color; ctx.beginPath(); ctx.arc(x,y,3,0,Math.PI*2); ctx.fill(); ctx.strokeStyle='#fff'; ctx.lineWidth=1; ctx.stroke() }
      return
    }
