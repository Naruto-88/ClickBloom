...
import { DateRange, getGscSafeDate, fmtDateISO, getPresets, buildDaysRange, buildMonthRange } from "@/lib/dates"
...
export default function PerformanceClient() {
  ...
  const [range, setRange] = useState<DateRange>(() => buildDaysRange(30))
  ...
  const rangeToKey = (value: DateRange): '7d' | '30d' | 'lastm' | '3m' | '6m' | '1y' | 'custom' => {
    for (const option of presetOptions) {
      ...
      if (fmtDateISO(value.from) === fmtDateISO(from) && fmtDateISO(value.to) === fmtDateISO(to)) {
        return option.key
      }
    }
    return 'custom'
  ...
  useEffect(() => {
    ...
    const loadSite = async (id: string) => {
      ...
      if (ga4) {
        ...
        ga4Promise = fetch('/api/google/ga4/acquisition', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ property: ga4, start: fmtDateISO(gaStart), end: fmtDateISO(gaEnd), prevStart: fmtDateISO(gaPrevStart), prevEnd: fmtDateISO(gaPrevEnd) }) })
      }
      ...
      if (gsc) {
        ...
        const currentSearchPromise = fetch(`/api/google/gsc/search?${qs({ site: gsc, start: fmtDateISO(gStart), end: fmtDateISO(gEnd) })}`)
        const prevSearchPromise = fetch(`/api/google/gsc/search?${qs({ site: gsc, start: fmtDateISO(gPrevStart), end: fmtDateISO(gPrevEnd) })}`)
        const currentQueriesPromise = fetch(`/api/google/gsc/queries?${qs({ site: gsc, start: fmtDateISO(gStart), end: fmtDateISO(gEnd), rowLimit: 25000 })}`)
        const prevQueriesPromise = fetch(`/api/google/gsc/queries?${qs({ site: gsc, start: fmtDateISO(gPrevStart), end: fmtDateISO(gPrevEnd), rowLimit: 25000 })}`)
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          {snapshotTs && (
            <span className="muted" style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
              <span className="badge" style={{ background: '#1a1a33', color: '#94a3b8', border: '1px solid #2b2b47' }}>Data as of {getGscYesterday().toLocaleDateString()}</span>
...
function QueryDetails({ siteId, term, range, data, setData, gscSite }: { siteId: string, term: string, range: DateRange, data: any, setData: (x: any) => void, gscSite?: string }) {
  ...
  const setLastDays = (days: number) => { const r = buildDaysRange(days); setQRange(r) }
  useEffect(() => {
    (async () => {
      setLoading(true)
      try {
        const tr = await fetch(`/api/google/gsc/query-trend?${qs({ site: gscSite, start: fmtDateISO(qRange.from), end: fmtDateISO(qRange.to), query: term })}`)
        ...
        const pr = await fetch(`/api/google/gsc/query-pages?${qs({ site: gscSite, start: fmtDateISO(qRange.from), end: fmtDateISO(qRange.to), query: term, rowLimit: 1000 })}`)
