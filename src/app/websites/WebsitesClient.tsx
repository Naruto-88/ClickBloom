...
export default function WebsitesClient() {
  ...
  useEffect(() => {
    setKeyInput(integ.wpToken || ''); if (integ.wpToken) {
  ...
  const exportCsv = () => {
    ...
    const rows = ['URL,Name,Industry']
    for (const w of sites) {
      rows.push(`${escape(w.url)},${escape(w.name)},${escape(w.industry || '')}`)
    }
    const blob = new Blob([rows.join('\n')], { type: 'text/csv;charset=utf-8' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = 'clickbloom_websites_export.csv'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url)
  }
      ...
      <div style={{ background: '#0b0f1a', color: '#e5e7eb', borderRadius: 12, padding: 16, boxShadow: '0 1px 2px rgba(0,0,0,.4)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button className="btn secondary" onClick={exportCsv} title="Export all websites to CSV">Export CSV</button>
            <button className="btn secondary" onClick={downloadTxtTemplate}>Download template</button>
          </div>
        </div>
        <div style={{ border: '1px solid #1f2937', borderRadius: 10, overflow: 'hidden' }}>
          ...
          {filtered.map(w => (
            <div key={w.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 160px', padding: '14px 12px', borderTop: '1px solid #1f2937', alignItems: 'center' }}>
              ...
              <div>
                <div style={{ fontWeight: 800 }} suppressHydrationWarning>{fmtDate((w as any).createdAt)}</div>
              </div>
