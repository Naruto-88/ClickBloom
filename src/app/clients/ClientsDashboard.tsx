...
function loadSites(): Website[] { if (typeof window === 'undefined') return []; try { return JSON.parse(localStorage.getItem('websites') || '[]') } catch { return [] } }
...
export default function ClientsDashboard() {
  ...
  const presets = useMemo(() => ([
    { key: '7d', label: '7 Days', range: buildDaysRange(7) },
    { key: '14d', label: '14 Days', range: buildDaysRange(14) },
    { key: '30d', label: '30 Days', range: buildDaysRange(30) },
    { key: 'lastm', label: 'Last Month', range: buildMonthRange(-1) },
    { key: '3m', label: '3 Months', range: buildDaysRange(90) },
    { key: '6m', label: '6 Months', range: buildDaysRange(180) },
    { key: '12m', label: '12 Months', range: buildDaysRange(365) },
  ]), [])
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const start = fmtDateISO(range.from)
        let endDate = range.to; const y = getGscSafeDate(); if (endDate > y) endDate = y
        const end = fmtDateISO(endDate)
        const days = Math.max(1, Math.round((endDate.getTime() - range.from.getTime()) / 86400000) + 1)
        const prevEnd = new Date(range.from); prevEnd.setDate(prevEnd.getDate() - 1)
        const prevStart = new Date(prevEnd); prevStart.setDate(prevEnd.getDate() - (days - 1))
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button className="btn secondary" disabled={warmBusy} onClick={async () => {
              try {
                setWarmBusy(true)
                const presetsLocal = getPresets()
                for (const p of presetsLocal) {
                  const r = p.range
                  const start = fmtDateISO(r.from)
                  let endDate = r.to;
                  const y = getGscSafeDate();
                  if (endDate > y) endDate = y
                  const end = fmtDateISO(endDate)
                  const days = Math.max(1, Math.round((endDate.getTime() - r.from.getTime()) / 86400000) + 1)
                  const prevEnd = new Date(r.from); prevEnd.setDate(prevEnd.getDate() - 1)
                  const prevStart = new Date(prevEnd); prevStart.setDate(prevEnd.getDate() - (days - 1))
                  const pStart = fmtDateISO(prevStart)
                  const pEnd = fmtDateISO(prevEnd)
                  const out: Row[] = []
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const resite = async () => {
                        ...
                        const y = getGscSafeDate()
                        let start = new Date(range.from), end = new Date(range.to); if (end > y) end = y
                        ...
                        const prevStart = new Date(prevEnd); prevStart.setDate(prevEnd.getDate() - (days - 1))
                        const pStart = fmtDateISO(prevStart), pEnd = fmtDateISO(prevEnd)
                        if (integ.gscSite) {
                          try { gCurr = await fetchGscTotals(integ.gscSite, fmtDateISO(start), fmtDateISO(end), r.name || r.url) } catch { }
                          try { gPrev = await fetchGscTotals(integ.gscSite, pStart, pEnd, r.name || r.url) } catch { }
                        }
                        let orgSessions = 0, orgSessionsPrev = 0, orgUsers = 0, orgUsersPrev = 0
                        if (integ.ga4Property) {
                          try { orgSessions = await fetchGa4OrganicSessions(integ.ga4Property, fmtDateISO(start), fmtDateISO(end), r.name || r.url) } catch { }
                          try { orgSessionsPrev = await fetchGa4OrganicSessions(integ.ga4Property, pStart, pEnd, r.name || r.url) } catch { }
                          try { orgUsers = await fetchGa4OrganicUsers(integ.ga4Property, fmtDateISO(start), fmtDateISO(end), r.name || r.url) } catch { }
