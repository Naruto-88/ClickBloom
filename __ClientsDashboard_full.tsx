...
export default function ClientsDashboard(){
  ...
  async function fetchGa4OrganicSessions(property: string, start: string, end: string){
    try{
  ...
  async function fetchGa4OrganicUsers(property: string, start: string, end: string){
    try{
    ...
    try{
      ...
      for(const w of sites){
        ...
        const status = decideStatus(
          { clicks: gCurr.clicks, impressions: gCurr.impressions, pos: gCurr.position||0, sessions: orgSessions },
          { clicks: gPrev.clicks, impressions: gPrev.impressions, pos: gPrev.position||0, sessions: orgSessionsPrev }
        )
  ...
  const sorted = useMemo(()=>{
    ...
    const score = (r:Row)=>{
      ...
      const dSess = pct(r.organicSessions, r.organicSessionsPrev)
    ...
    <>
      ...
      <div className="card" style={{padding:12, position:'relative'}}>
        ...
        <div className="clients-grid cols" style={{padding:'6px 8px', borderBottom:'1px solid #23233a'}}> 
          ...
          <button className="th" onClick={()=>{ setSortBy('users'); setSortDir(sortBy==='users' && sortDir==='desc' ? 'asc':'desc') }}>
          ...
          <button className="th" onClick={()=>{ setSortBy('sessions'); setSortDir(sortBy==='sessions' && sortDir==='desc' ? 'asc':'desc') }}>
            Organic Users • Sessions <span className="caret">{sortBy==='sessions' ? (sortDir==='desc'?'▼':'▲') : ''}</span>
          </button>
