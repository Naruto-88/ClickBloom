...
async function crawlSite(startUrl: string, maxPages: number, maxDepth: number) {
  ...
  while (q.length > 0 && results.length < maxPages) {
    // Increased concurrency for faster crawling
    ...
    await Promise.all(batch.map(async item => {
      ...
      try {
        ...
        results.push({
          ...
          schemaCount,
          canonical,
          images: { total: totalImgs, withAlt }
        })
        ...
        $('a[href]').each((_, el) => {
          try {
            ...
            const u = norm(abs)
            if (!seen.has(u) && !q.some(x => x.url === u)) {
              q.push({ url: u, depth: item.depth + 1 })
            }
          } catch { }
        })
      } catch (e) {
        console.error(`Failed to crawl ${item.url}:`, e)
      }
    }))
  }
  return results
