"use client"
import { GSC_LAG_DAYS, fmtDateISO, getGscSafeDate as yesterdayGsc } from "./dates"

const SNAPSHOT_EVENT_NAME = 'clickbloom:snapshot-updated'
export const SNAPSHOT_TTL_MS = 12 * 60 * 60 * 1000 // 12 hours
export const GSC_SAFE_LAG_DAYS = GSC_LAG_DAYS
...
async function fetchClientsPreset(key: string, from: Date, to: Date) {
  ...
  const start = fmtDateISO(clampFrom)
  const end = fmtDateISO(clampTo)
